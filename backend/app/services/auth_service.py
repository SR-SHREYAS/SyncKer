...
class AuthService:
    ...
    def RegisterUser(self, *, email: str, username: str, password: str) -> AuthResult:
        ...
        if self.user_repo.get_by_email(email):
            logger.error("auth register blocked: email already exists")
            raise ConflictError("email is already registered")

        if self.user_repo.get_by_username(username):
            logger.error("auth register blocked: username already exists")
        ...
        logger.info("auth register service completed")
    ...
    def LoginUser(self, *, email: str, password: str) -> AuthResult:
        ...
        if user is None:
            logger.error("auth login blocked: user not found")
            raise AuthenticationError("invalid email or password")

        if not user.is_active:
            logger.error("auth login blocked: inactive account")
            raise AuthenticationError("user account is inactive")

        if not verify_password(password, user.password_hash):
            logger.error("auth login blocked: password mismatch")
            raise AuthenticationError("invalid email or password")

        access_token, expires_at = create_access_token(user.id)
        logger.info("auth login service completed")
        return AuthResult(user=user, access_token=access_token, expires_at=expires_at)
