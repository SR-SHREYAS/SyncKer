    ...
    while current_date <= window_end_at.date():
        for block in routine_blocks:
            ...
            blocked_ranges.append(
                (
                    datetime.combine(
                        current_date, block.start_time, tzinfo=window_start_at.tzinfo
                    ),
                    datetime.combine(
                        current_date, block.end_time, tzinfo=window_start_at.tzinfo
                    ),
                )
            )
    ...
    *,
    ...
    while current_date <= window_end_at.date():
        for block in availability_blocks:
            ...
            if is_slot_blocked(
                slot_start=slot_start, slot_end=slot_end, blocked_ranges=blocked_ranges
            ):
                continue
