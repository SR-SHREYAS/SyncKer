...
def create_skill(payload: SkillCreateRequest, handler: Annotated[SkillHandler, Depends(get_skill_handler)]) -> SkillResponse:
    """Create one skill in the shared catalog."""
    return handler.createSkillCatalogEntry(payload)


@router.get("", response_model=list[SkillResponse])
def list_skills(handler: Annotated[SkillHandler, Depends(get_skill_handler)]) -> list[SkillResponse]:
...
def attach_skill_to_me(payload: UserSkillCreateRequest, current_user_id: CurrentUserId, handler: Annotated[SkillHandler, Depends(get_skill_handler)]) -> UserSkillResponse:
...
def list_my_skills(current_user_id: CurrentUserId, handler: Annotated[SkillHandler, Depends(get_skill_handler)]) -> list[UserSkillResponse]:
