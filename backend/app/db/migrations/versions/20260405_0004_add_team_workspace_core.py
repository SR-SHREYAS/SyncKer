...
def upgrade() -> None:
    ...
    op.add_column("session_suggestions", sa.Column("team_id", sa.Integer(), nullable=True))
    op.create_index(op.f("ix_session_suggestions_team_id"), "session_suggestions", ["team_id"], unique=False)
...
def downgrade() -> None:
    op.drop_constraint("fk_session_suggestions_team_id_teams", "session_suggestions", type_="foreignkey")
    op.drop_index(op.f("ix_session_suggestions_team_id"), table_name="session_suggestions")
